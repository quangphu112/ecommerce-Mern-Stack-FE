...
const Cart = () => {
  ...
  const handleInc = (id) => {
    const updateInc = cart.map((item) => {
      if (item.data.id === id && item.quantity < 10) {
        return {
          ...item,
  ...
  const handleDec = (id) => {
    const handleDec = cart.map((item) => {
      if (item.data.id === id && item.quantity > 1) {
        return {
          ...item,
          quantity: item.quantity - 1,
        };
    ...
    localStorage.setItem("cart", JSON.stringify(handleDec));
    navigate("/cart");
  };
                ...
                return (
                  <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                    <div className="flex w-2/5">
                      <div className="w-20">
                        <img className="h-24" src={item?.data?.image} />
                      </div>
                    ...
                    <div className="flex justify-center w-1/5">
                      <svg
                        ...
                        viewBox="0 0 448 512"
                        onClick={() => handleDec(item?.data?.id)}
                      >
                      ...
                      <svg
                        className="fill-current text-gray-600 w-3 cursor-pointer"
                        onClick={() => handleInc(item?.data?.id)}
                        viewBox="0 0 448 512"
                      >
